...
class TestAccount(TestCase):
    def test_account(self):
        ...
        o = td.Order(symbol=symbol, oc=const.OC.O, price=10, order_num=1000, dt=datetime.now())
        a.on_ordered(x=td.Ordered(order=o, dt=datetime.now()))
        self.assertTrue(a.frozen == 10 * 1000)
        a.on_trade(x=td.Trade(dt=datetime.now(), order=o, price=o.price, num=o.order_num))
        self.assertTrue(
            a.frozen == 0 and
            a.cash == (cash - max(1000 * 10 * 0.00025, 5))
        )
        a.on_tick(x=md.Tick(symbol=symbol, dt=datetime.now(), price=20, volume=100))
        self.assertTrue(a.profit == (20 - 10) * 1000)

        o = td.Order(symbol=symbol, oc=const.OC.C, price=20, order_num=1000, dt=datetime.now())
        a.on_ordered(x=td.Ordered(order=o, dt=datetime.now()))
        a.on_trade(x=td.Trade(dt=datetime.now(), order=o, price=o.price, num=o.order_num))
        self.assertTrue(
            a.frozen == 0 and
            ...
            a.cash == (
                cash -
                max(1000 * 10 * 0.00025, 5) -
                1000 * 20 * 0.001 -
                max(1000 * 20 * 0.00025, 5) +
                (20 - 10) * 1000
            )
        )
